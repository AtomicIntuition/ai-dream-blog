            ...
            {categories.map((category) => {
              ...
              return (
                <Link
                  ...
                  className="flex items-center gap-2 px-4 py-2 text-sm text-slate-400 hover:text-white hover:bg-white/5 rounded-full transition-all whitespace-nowrap"
                >
            ...
            <div className="grid lg:grid-cols-5 gap-6 lg:gap-8">
              ...
              <div className="lg:col-span-3">
                <Link href={`/post/${heroPost.slug}`} className="group block">
                  <article className="relative h-full min-h-[400px] lg:min-h-[500px] rounded-3xl overflow-hidden glass-card">
                    {/* Gradient overlay */}
                    <div className="absolute inset-0 bg-gradient-to-t from-[rgb(10,10,20)] via-[rgb(10,10,20)]/60 to-transparent z-10" />
                    ...
                    <div className="absolute inset-0 z-20 flex flex-col justify-end p-6 md:p-8">
                      <div className="space-y-4">
                        ...
                        <h1 className="text-3xl md:text-4xl lg:text-5xl font-display font-bold text-white leading-tight group-hover:text-dream-300 transition-colors">
                          {heroPost.title}
                        </h1>

                        {heroPost.subtitle && (
                          <p className="text-lg md:text-xl text-slate-300 line-clamp-2">
                            {heroPost.subtitle}
                          </p>
                        )}
              ...
              <div className="lg:col-span-2 flex flex-col gap-4">
                {secondaryPosts.map((post) => (
                  <Link key={post.id} href={`/post/${post.slug}`} className="group flex-1">
                    <article className="h-full glass-card p-5 rounded-2xl hover:border-white/20 transition-all">
                      <span className={`category-badge category-${post.category} text-xs mb-3`}>
                        {getCategoryLabel(post.category)}
                      </span>
                      <h2 className="text-lg font-semibold text-white mb-2 group-hover:text-dream-300 transition-colors line-clamp-2">
                        {post.title}
                      </h2>
                ...
                <div className="glass-card p-5 rounded-2xl bg-gradient-to-br from-dream-500/10 to-aurora-500/10">
                  ...
                  <Link
                    ...
                    className="inline-flex items-center gap-1.5 text-sm font-medium text-dream-400 hover:text-dream-300 transition-colors"
                  >
                    Try Dream Analysis
        ...
        <section className="py-12 border-y border-white/5 bg-white/[0.02]">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="flex items-center gap-2 mb-6">
              <TrendingUp className="h-5 w-5 text-dream-500" />
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {featuredPosts.map((post, index) => (
                <Link key={post.id} href={`/post/${post.slug}`} className="group flex items-start gap-4">
                  <span className="text-4xl font-display font-bold text-dream-500/30 group-hover:text-dream-500/50 transition-colors">
                    {String(index + 1).padStart(2, '0')}
                  </span>
                  <div className="flex-1 min-w-0">
                    <h3 className="font-medium text-white group-hover:text-dream-300 transition-colors line-clamp-2 mb-1">
                      {post.title}
                    </h3>
      ...
      <section id="latest" className="py-16 md:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-10">
            ...
            <Link
              href="/archive"
              className="flex items-center gap-1 text-sm font-medium text-dream-400 hover:text-dream-300 transition-colors"
            >
              View all
            ...
            {categories.map((category) => {
              ...
              return (
                <Link
                  key={category.slug}
                  href={`/category/${category.slug}`}
                  className="group glass-card p-6 rounded-2xl hover:border-white/20 transition-all flex items-start gap-4"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <ChevronRight className="h-4 w-4 text-slate-500 group-hover:text-dream-400 group-hover:translate-x-0.5 transition-all" />
                    </div>
      ...
      <section className="py-16 border-t border-white/5">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 text-center">
          ...
          <a
            ...
            className="inline-flex items-center gap-2 text-sm text-dream-400 hover:text-dream-300 transition-colors"
          >
            Follow on Twitter
